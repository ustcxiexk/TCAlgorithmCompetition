    ...
    for i in range(train.shape[0]):
        ...
        for item in feature_list[i].split(" "):
            if item not in feature_label_count.keys():
                ...
                score = score * (
                1 - (feature_label_count[item] + hyper.alpha) / (feature_count[item] + hyper.alpha + hyper.beta))
...
# train, predict = nlp_feature_score("appIdInstall", train, predict)
# train,predict = nlp_feature_score("interest1", train, predict)
...
# 交叉特征
# data['aid_age'] = data['aid'].astype(str) + '_' + data['age'].astype(str)
# print (data['aid_age'][1:10])
...
aid_gender_count = data.groupby(['aid', 'education']).size().reset_index().rename(columns={0: 'aid_education_count'})
data = pd.merge(data, aid_gender_count, 'left', on=['aid', 'education'])
# aid_age_count = data.groupby(['adCategoryId', 'age']).size().reset_index().rename(columns={0: 'age_adCategoryId_count'})
# data = pd.merge(data, aid_age_count, 'left', on=['adCategoryId', 'age'])
...
enc = OneHotEncoder()
raw_feature = ['creativeSize', "aid_age_count",  'aid_gender_count', 'aid_education_count',
               'campaignId_active_aid', 'appIdAction_score', 'interest1_score', 'interest2_score']
...
def LGB_predict(train_x, train_y, test_x, res):
    ...
    clf = lgb.LGBMClassifier(
        ...
        max_depth=-1, n_estimators=11000, objective='binary',
